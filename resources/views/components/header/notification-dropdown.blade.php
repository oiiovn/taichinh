            ...
            setInterval(() => {
                fetch('{{ route('notifications.unread-count') }}', { headers: { 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json' }, credentials: 'same-origin' })
                    ...
                    .then(d => {
                        ...
                        if (c > this.lastCount) {
                            if (typeof Alpine !== 'undefined' && Alpine.store('notificationFlash') !== undefined) Alpine.store('notificationFlash', true);
                            setTimeout(() => { if (Alpine.store('notificationFlash') !== undefined) Alpine.store('notificationFlash', false); }, 5000);
                            this.notifying = true;
                        }
                        this.lastCount = c;
                    }).catch(() => {});
...
>
    <!-- Notification Button (flash 5s khi có thông báo mới) -->
    <button
        type="button"
        class="relative flex items-center justify-center h-11 w-11 rounded-full border bg-white text-gray-500 transition-colors hover:bg-gray-100 hover:text-gray-700 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-400 dark:hover:bg-gray-800 dark:hover:text-white"
