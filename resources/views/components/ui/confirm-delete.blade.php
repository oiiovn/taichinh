...
@php
    $isControlled = $controlled === true || ($controlled === null && $openVar !== null);
    $openVar = $openVar ?? 'open';
@endphp

@if($isControlled)
    {{-- Chế độ controlled (vd: Đóng hợp đồng) — parent quản lý openVar --}}
...
@else
    {{-- Modal toàn cục: $dispatch('confirm-delete-open', { formId, message }) hoặc { message, resolve } --}}
    <div
        x-data="{
            open: false,
            formId: null,
            message: @js($defaultMessage),
            defaultMessage: @js($defaultMessage),
            resolve: null,
            openModal(detail = {}) {
                this.formId = detail.formId ?? null;
                this.resolve = typeof detail.resolve === 'function' ? detail.resolve : null;
                this.message = detail.message || this.defaultMessage;
                this.open = true;
            },
            close(ok = false) {
        ...
        @confirm-delete-open.window="openModal($event.detail || {})"
        x-cloak
    >
        <div x-show="open"
            class="fixed inset-0 z-[100000] flex items-center justify-center overflow-y-auto p-4"
            ...
            <div x-show="open"
                ...
                <p class="mb-6 text-theme-sm text-gray-600 dark:text-gray-300" x-text="message"></p>
                <div class="flex justify-end gap-2">
                    <button type="button" @click="close(false)"
                        class="rounded-lg border border-gray-300 bg-white px-4 py-2 text-theme-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700">
                        {{ $cancelText }}
