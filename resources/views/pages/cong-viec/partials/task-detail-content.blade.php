    ...
    <div class="px-5 py-5 md:px-6 md:py-6">
        ...
        <div class="mt-6 flex flex-wrap items-center gap-2">
            @if(!$inModal)
                ...
                @endif
                <form id="form-delete-task-{{ $task->id }}" action="{{ route('cong-viec.tasks.destroy', $task->id) }}" method="POST" class="inline">
                    @csrf
                    @method('DELETE')
                    <button type="button"
                        @click="$dispatch('confirm-delete-open', { formId: 'form-delete-task-{{ $task->id }}', message: 'Xóa công việc này? Không thể hoàn tác.' })"
                        class="inline-flex items-center gap-1.5 rounded-lg border border-red-300 bg-white px-3 py-2 text-sm font-medium text-red-700 shadow-sm hover:bg-red-50 dark:border-red-800 dark:bg-gray-700 dark:text-red-400 dark:hover:bg-red-900/20">Xóa</button>
                </form>
            @endif
        </div>
    </div>
