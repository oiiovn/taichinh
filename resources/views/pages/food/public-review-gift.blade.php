    ...
    >
        ...
        >
            ...
            <div class="h-full w-full overflow-hidden rounded-none bg-white p-5 shadow-xl dark:bg-gray-900 sm:h-auto sm:max-h-[90vh] sm:max-w-md sm:overflow-y-auto sm:rounded-xl">
                ...
                @if(session('gift_code'))
                    <div class="mt-2 rounded-md border border-amber-300 bg-amber-50 px-2 py-1 text-sm font-semibold text-amber-700 dark:border-amber-500/40 dark:bg-amber-900/30 dark:text-amber-200">
                        ...
                        <button
                            ...
                            @click="navigator.clipboard && navigator.clipboard.writeText('{{ session('gift_code') }} Tặng {{ session('gift_item_name', 'Bánh Tráng Trộn') }}')"
                ...
                @endif
                <div class="mt-6 flex items-center gap-2 sm:mt-4 sm:justify-end">
                    @if(session('gift_branch_link'))
                        ...
                        >Mua ngay</a>
                    @endif
                    <button type="button" @click="openGift = false" class="flex-1 rounded-lg bg-brand-600 px-4 py-3 text-sm font-medium text-white hover:bg-brand-700 sm:flex-none sm:py-2">Đã hiểu</button>
                </div>
            </div>
        </div>
    </div>
