        ...
        <div class="flex flex-wrap items-center gap-2">
            @if($canManage ?? true)
                ...
                <form action="{{ route('food.bao-cao-ban-hang.cong-no.store', $report) }}" method="POST" class="inline" x-data="{
                    ...
                    @csrf
                    <button type="button" @click="open = true; deductionAmount = 0; additionAmount = 0; onlyTienCong = false; onlyTienCongKhungGio = false" class="rounded-lg border border-amber-300 px-4 py-2 text-sm font-medium text-amber-700 hover:bg-amber-50 dark:border-amber-700 dark:text-amber-400 dark:hover:bg-amber-900/20">Xử lý công nợ</button>
