...
<div class="space-y-6">
    <div class="flex flex-wrap items-center gap-2">
        ...
        <h1 class="text-theme-xl font-semibold text-gray-900 dark:text-white">Sửa chi ước tính</h1>
    </div>
    ...
    <div class="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-900 dark:text-white max-w-md">
        <form method="POST" action="{{ route('thu-chi.expense.update', $expense->id) }}" class="space-y-3">
            @csrf
            @method('PUT')
            ...
            <input type="text" name="amount" value="{{ old('amount', (int) $expense->amount) }}" placeholder="Số tiền" required inputmode="numeric" data-format-vnd
                class="w-full rounded-lg border border-gray-300 px-3 py-2 text-theme-sm dark:border-gray-600 dark:bg-gray-800 dark:text-white">
            <input type="text" name="category" value="{{ old('category', $expense->category) }}" placeholder="Danh mục"
                class="w-full rounded-lg border border-gray-300 px-3 py-2 text-theme-sm dark:border-gray-600 dark:bg-gray-800 dark:text-white">
            <input type="text" name="note" value="{{ old('note', $expense->note) }}" placeholder="Ghi chú"
                class="w-full rounded-lg border border-gray-300 px-3 py-2 text-theme-sm dark:border-gray-600 dark:bg-gray-800 dark:text-white">
