                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    @if ($u->id !== auth()->id())
                                        <form id="form-delete-user-{{ $u->id }}" action="{{ route('admin.users.destroy', $u) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" @click="$dispatch('confirm-delete-open', { formId: 'form-delete-user-{{ $u->id }}', message: 'Bạn có chắc muốn xóa user này?' })" class="rounded-lg border border-red-200 px-3 py-1.5 text-sm text-red-600 hover:bg-red-50 dark:border-red-800 dark:text-red-400 dark:hover:bg-red-900/20">Xóa</button>
                                        </form>
                                    @endif
                                </div>
                            </td>
